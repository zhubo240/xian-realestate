...
│   ├── gaoxin_newhouse_all.csv     # 新房完整数据（171条）
...
| 脚本 | 功能 | 输入 | 输出 |
|------|------|------|------|
| `scripts/scrape_gaoxin_esf.py` | 抓取二手房小区数据 | fang.com 56页 | `gaoxin_esf_all.csv` |
| `scripts/geocode_communities.py` | 二手房地理编码+片区分类 | `gaoxin_esf_all.csv` | `gaoxin_esf_all_geo.csv` |
| `scripts/geocode_newhouse.py` | 新房地理编码+片区分类 | `gaoxin_newhouse_all.csv` | `gaoxin_newhouse_all_geo.csv` |
| `scripts/merge_and_map.py` | 合并数据+生成地图 | `*_geo.csv` | `gaoxin_merged_all.csv` + `gaoxin_map.html` |
